           	...
           	<div class="box_center pt10 pb10">
             	...
             	<table class="form_table" border="0" cellpadding="0" cellspacing="0">
               	<tr>
               		...
               		<td>描述</td>
               		<td><input type="text" name="article.Description" class="input-text lh25" size="20" value="${entity.description }"></td>
...
<div id="table" class="mt10">
	<div class="box span10 oh">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="list_table">
            ...
            <c:forEach items="${pager.datas}" var="d" varStatus="v">
				<tr class="tr">
					...
					<td class="td_center">
						<c:if test="${fn:contains(d.picUrl,'http')}"><img alt="" src="${d.picUrl }" width="100px;" title="${d.picUrl }"></c:if>
						<c:if test="${!fn:contains(d.picUrl,'http')}"><img alt="" src="${ctx }/${d.picUrl }" width="100px;" title="${d.picUrl }"></c:if>
					</td>
					<td class="td_center">
						<c:if test="${fn:length(d.url) gt 40}"><a title="${d.url }"><c:out value="${fn:substring(d.url, 0, 40)}..." /></a></c:if>
		...
		<jsp:include page="../common/pager.jsp">
			<jsp:param value="${ctx }/article/list.html" name="url"/>
...
$(function(){
	$(".add_").click(function(){
        ymPrompt.win({title:'添加',height:'430',width:'500',dragOut:false,iframe:true,message:'${ctx}/article/toAdd.html',handler:refresh});
    })
    
	$(".edit_").click(function(){
        var id = $(this).attr("data");
        ymPrompt.win({title:'添加',height:'430',width:'500',dragOut:false,iframe:true,message:'${ctx}/article/toUpdate.html?article.id=' + id,handler:refresh});
    })
...
function handlerDel(tp){
	if(tp=='ok'){
		$.ajax({
            type : "GET",
            url : '${ctx}/article/delete.html?article.id=' + delete_id,
            dataType : "text",
...
// 刷新页面
function refresh(){
	self.location.href="${ctx}/article/list.html";
}
</script>
</body>
</html>